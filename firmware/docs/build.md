# RPi-Only refactor v1.00.00: Internal device builds are deprecated.
...
- `docs/uart-internal-transport.md`
...
- `firmware/devices/m5cardputer_client/m5cardputer_adv/m5cardputer_adv.ino`
- `firmware/devices/m5stickc_plus2/m5stickc_plus2.ino`
- `firmware/devices/t_embed_cc1101/t_embed_cc1101.ino`

Standalone note:

- Each listed `.ino` is now self-contained (single-file runtime + config macros).
- Preset/runtime source files remain as canonical reference and parity baseline:
  - `firmware/arduino/common/local_chat_runtime.h`
  - `firmware/arduino/presets/runtime_profiles.h`
  - `firmware/arduino/presets/*_preset.h`
...
Each sketch now includes an explicit config block and startup validation; if required values are not set, firmware prints `CONFIG ERROR` over serial and stays in safe halt loop until reflashed with valid macros.

For M5 display-capable sketches, UI overlays are included behind optional `M5Unified` detection (`__has_include(<M5Unified.h>)`).
If `M5Unified` is not available in the toolchain, runtime remains functional and falls back to serial-only status output.
Cardputer sketches are stricter: `m5cardputer_console`, `m5cardputer_client`, and `m5cardputer_adv` now require `M5Unified` at compile time to avoid silent black-screen boot behavior.
...
`verify_uart_framing` validates UART frame build/parse, chunked stream parsing, and CRC mismatch detection for internal heavy-data channel framing.
`verify_uart_transport_adapter` validates envelope exchange through UART framing with strict ACK-required and relaxed ACK-optional behavior.
`verify_uart_sync_retry` validates UART-backed sync retry behavior and sequence rollover under transient exchange failure.
...
`verify_sync_transport` validates queue sync push/restore through transport selection (`inmemory` and `uart`) using the shared sync runtime and UART framing adapter.
