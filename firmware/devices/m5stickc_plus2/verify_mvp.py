...
from .models import CompactScreen, ConnectionState
from .server_api import CommandSender, CompactAuthGateway
...
def main() -> None:
    ...
    db_name = f"local_chat_m5stickc_shell_test_{uuid.uuid4().hex}.db"
    ...
    with TestClient(app) as client:
        ...
        conn.execute(
            ...
            (
                "m5stick-user",
                hash_password("m5stick-secret-123"),
                now_ms,
                now_ms,
            ),
        )
        ...
        gateway = CompactAuthGateway(sender)
        config = M5StickCPlus2Config(client_kind="device")
        controller = M5StickCPlus2Controller(config=config, gateway=gateway)

        connected = controller.start_shell(now_ms=9101)
        logged_in = controller.secure_login(
            login="m5stick-user",
            password="m5stick-secret-123",
            now_ms=9102,
        )
        if logged_in.session is None:
            raise RuntimeError("secure_login did not create session")
        opened_chat = controller.open_screen(screen=CompactScreen.CHAT, now_ms=9103)
        opened_blog = controller.open_screen(screen=CompactScreen.BLOG, now_ms=9104)
        resumed = controller.resume_session(session_token=logged_in.session.token, now_ms=9105)
        mode_payload = gateway.mode()
        logged_out = controller.logout(now_ms=9106)
        ...
        print("COMMAND_MAP_COUNT", len(M5STICKC_PLUS2_COMMANDS))
        ...
        if opened_chat.active_screen != CompactScreen.CHAT:
            raise RuntimeError("chat navigation failed")
        if opened_blog.active_screen != CompactScreen.BLOG:
...
def _verify_command_map(route_map: dict[str, set[str]]) -> None:
    for command in M5STICKC_PLUS2_COMMANDS:
    ...
    if "/auth/guest" in command_path_set():
        raise RuntimeError("compact command map must not include guest login path")
