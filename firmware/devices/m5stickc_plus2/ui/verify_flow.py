...
def main() -> None:
    ...
    db_name = f"local_chat_m5stick_client_flow_{uuid.uuid4().hex}.db"
    ...
    with TestClient(app) as client:
        ...
        conn.execute(
            ...
            (
                "m5stick-admin",
                hash_password("m5stick-admin-secret"),
                now_ms,
                now_ms,
            ),
        )
        conn.execute(
            ...
            (
                "m5stick-client-user",
                hash_password("m5stick-client-secret"),
                now_ms + 1,
                now_ms + 1,
            ),
        )
        ...
        published = client.post(
            ...
            json={
                ...
                "title": "M5Stick Compact Blog",
                "body_text": "Post for compact client verification",
            },
        )
        ...
        controller = M5StickCPlus2ClientController(
            config=M5StickCPlus2Config(profile_id="m5stickc_plus2", client_kind="device"),
            sender=sender,
        )

        session = controller.secure_login(login="m5stick-client-user", password="m5stick-client-secret")
        ...
        sent = controller.send_text(
            ...
            body_text="m5stick compact text message",
            client_message_id="m5stick-msg-1",
        )
        ...
        print("COMMAND_MAP_COUNT", len(M5STICKC_PLUS2_CLIENT_COMMANDS))
        ...
        if session.role != "user":
            raise RuntimeError("compact client login role mismatch")
        if chats.count < 1:
...
def _verify_command_map(route_map: dict[str, set[str]]) -> None:
    for command in M5STICKC_PLUS2_CLIENT_COMMANDS:
    ...
    if overlap:
        raise RuntimeError(f"compact client command map contains forbidden paths: {overlap}")
