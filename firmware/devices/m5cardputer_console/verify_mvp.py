...
from .models import ConnectionState, NavigationScreen
...
def main() -> None:
    ...
    db_name = f"local_chat_m5cardputer_console_test_{uuid.uuid4().hex}.db"
    ...
    with TestClient(app) as client:
        ...
        conn.execute(
            ...
            (
                "console-user",
                hash_password("console-secret-123"),
                now_ms,
                now_ms,
            ),
        )
        ...
        gateway = ConsoleAuthGateway(sender)
        config = M5CardputerConsoleConfig(client_kind="device")
        controller = M5CardputerConsoleController(config=config, gateway=gateway)

        connected = controller.start_shell(now_ms=5001)
        logged_in = controller.secure_login(
            login="console-user",
            password="console-secret-123",
            now_ms=5002,
        )
        if logged_in.session is None:
            raise RuntimeError("secure_login did not create session")
        opened_chat = controller.open_screen(screen=NavigationScreen.CHAT, now_ms=5003)
        opened_blog = controller.open_screen(screen=NavigationScreen.BLOG, now_ms=5004)
        resumed = controller.resume_session(session_token=logged_in.session.token, now_ms=5005)
        mode_payload = gateway.mode()
        logged_out = controller.logout(now_ms=5006)
        ...
        print("COMMAND_MAP_COUNT", len(M5CARDPUTER_CONSOLE_COMMANDS))
        ...
        if logged_in.session.role != "user":
            raise RuntimeError("secure_login returned unexpected role")
        if opened_chat.active_screen != NavigationScreen.CHAT:
            raise RuntimeError("chat navigation failed")
        if opened_blog.active_screen != NavigationScreen.BLOG:
...
def _verify_command_map(route_map: dict[str, set[str]]) -> None:
    for command in M5CARDPUTER_CONSOLE_COMMANDS:
    ...
    forbidden = command_path_set() & {"/auth/guest"}
    if forbidden:
        raise RuntimeError(f"console command map must not include guest login paths: {sorted(forbidden)}")
