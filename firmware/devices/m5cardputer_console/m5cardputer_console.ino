/*
 * M5Cardputer built-in console firmware (standalone single-file sketch).
 *
 ...
 * - Standalone preset equivalent of firmware/arduino/presets/m5cardputer_console_preset.h
 */
...
#ifndef LC_DEVICE_ID
#define LC_DEVICE_ID "m5cardputer-console-01"
#endif

#ifndef LC_DEVICE_TYPE
#define LC_DEVICE_TYPE "m5cardputer_console"
#endif
...
void render() {
    ...
    M5.Display.fillRect(0, 0, M5.Display.width(), 18, 0x028A);
    M5.Display.setTextColor(0xFFFF, 0x028A);
    M5.Display.setCursor(4, 5);
    M5.Display.print("Cardputer Console");
...
void begin(const char* device_id) {
    ...
    g_state.device_id = String(device_id == nullptr ? "cardputer" : device_id);
    auto cfg = M5.config();
    M5.begin(cfg);
...
public:
    ...
    void begin() {
        ui::set_phase("boot");
        Serial.println("m5cardputer console runtime: boot");
...
private:
    ...
    void maybe_send_chat_message() {
        ...
        const char* message_text = value_or_default(config_.chat_message_text, "hello from m5cardputer");
        const String payload = String("{\"session_token\":\"") + json_escape(session_token_) +
                               "\",\"body_text\":\"" + json_escape(String(message_text)) +
                               "\",\"client_message_id\":\"m5cardputer-" + String(millis()) + "\"}";
    ...
    void maybe_create_support_ticket() {
        ...
        const char* title = value_or_default(config_.support_ticket_title, "M5Cardputer ticket");
...
RuntimeConfig make_m5cardputer_console_standalone_config() {
...
RuntimeConfig kConfig = make_m5cardputer_console_standalone_config();
