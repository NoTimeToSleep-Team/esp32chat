...
from .models import ConnectionState, ConsoleSession, ConsoleShellState, NavigationScreen, disconnected_state
from .server_api import ConsoleAuthGateway


class M5CardputerConsoleShell:
    def __init__(self, gateway: ConsoleAuthGateway, config: M5CardputerConsoleConfig) -> None:
        self._gateway = gateway
        self._config = config
        self._state = disconnected_state()

    @property
    def state(self) -> ConsoleShellState:
        return self._state

    def connect(self, *, now_ms: int) -> ConsoleShellState:
        if self._state.connection_state == ConnectionState.AUTHENTICATED:
            return self._state

        self._state = ConsoleShellState(
...
def _parse_session_payload(payload: Mapping[str, Any]) -> ConsoleSession:
    if payload.get("status") != "ok":
