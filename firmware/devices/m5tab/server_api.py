...
class TelemetryGateway:
    ...
    def runtime_state(self, *, session_token: str | None) -> dict[str, Any] | None:
    ...
    def incidents(self, *, session_token: str | None, limit: int = 20) -> dict[str, Any] | None:
        token = (session_token or "").strip()
        if not token:
            return None
        return self._sender.send(
    ...
    def telemetry_bundle(self, *, session_token: str | None) -> dict[str, Any]:
        return {
            "health": self.health(),
            "readiness": self.readiness(),
            "mode": self.mode(),
            "runtime": self.runtime_state(session_token=session_token),
            "incidents": self.incidents(session_token=session_token),
        }
