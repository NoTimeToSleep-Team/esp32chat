/*
 * T-Embed CC1101 client firmware (standalone single-file sketch).
 *
 ...
 * - Standalone preset equivalent of firmware/arduino/presets/t_embed_cc1101_preset.h
 */
...
#ifndef LC_DEVICE_ID
#define LC_DEVICE_ID "t-embed-cc1101-01"
#endif

#ifndef LC_DEVICE_TYPE
#define LC_DEVICE_TYPE "t_embed_cc1101"
#endif
...
#define LC_CHAT_MESSAGE_TEXT "T-Embed CC1101 message"
#endif

#ifndef LC_SUPPORT_TITLE
#define LC_SUPPORT_TITLE "T-Embed support ticket"
#endif
...
UiState g_state = {
    false, false, false, false, "", "boot", "", "", -1, false, 0, 0, 0,
};
...
void render() {
    ...
    M5.Display.fillRect(0, 0, M5.Display.width(), 14, 0x0120);
    M5.Display.setTextColor(0xFFFF, 0x0120);
    M5.Display.setCursor(2, 2);
    M5.Display.print("T-Embed CC1101");

    draw_line(18, String("P:") + g_state.phase, 0xFFFF);
    draw_line(32, String("W:") + (g_state.wifi_connected ? "ok" : "down"), g_state.wifi_connected ? 0x07E0 : 0xF800);
    draw_line(46, String("A:") + (g_state.session_ready ? "ok" : "wait"), g_state.session_ready ? 0x07E0 : 0xFBE0);
    draw_line(60, String("ok=") + String(g_state.ok_count) + " f=" + String(g_state.fail_count), 0x9FF3);
    draw_line(74, String("[") + String(g_state.last_code) + "] " + g_state.last_label, 0xFFFF);
    draw_line(88, g_state.last_action, 0xBDF7);
    if(g_state.config_error) {
        draw_line(102, String("CFG ") + g_state.config_error_msg, 0xF800);
    }
#endif
}
...
void set_config_error(const char* message) {
    ...
    g_state.config_error_msg = String(message == nullptr ? "invalid" : message);
    g_state.phase = "config";
}
...
void tick() {
    ...
    if(now_ms - g_state.last_draw_ms >= 260) {
        render();
...
public:
    ...
    void begin() {
        ui::set_phase("boot");
        Serial.println("t-embed cc1101 runtime: boot");
...
private:
    ...
    void maybe_send_chat_message() {
        ...
        const char* message_text = value_or_default(config_.chat_message_text, "hello from t-embed cc1101");
        const String payload = String("{\"session_token\":\"") + json_escape(session_token_) +
                               "\",\"body_text\":\"" + json_escape(String(message_text)) +
                               "\",\"client_message_id\":\"tembed-" + String(millis()) + "\"}";
    ...
    void maybe_create_support_ticket() {
        ...
        const char* title = value_or_default(config_.support_ticket_title, "T-Embed support ticket");
...
RuntimeConfig make_t_embed_cc1101_standalone_config() {
...
RuntimeConfig kConfig = make_t_embed_cc1101_standalone_config();
