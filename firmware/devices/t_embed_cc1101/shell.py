...
from .models import ConnectionState, TEmbedScreen, TEmbedSession, TEmbedShellState, disconnected_state
from .server_api import TEmbedAuthGateway


class TEmbedCC1101Shell:
    def __init__(self, gateway: TEmbedAuthGateway, config: TEmbedCC1101Config) -> None:
        self._gateway = gateway
        self._config = config
        self._state = disconnected_state()

    @property
    def state(self) -> TEmbedShellState:
        return self._state

    def connect(self, *, now_ms: int) -> TEmbedShellState:
        if self._state.connection_state == ConnectionState.AUTHENTICATED:
            return self._state

        self._state = TEmbedShellState(
...
def _parse_session_payload(payload: Mapping[str, Any]) -> TEmbedSession:
    if payload.get("status") != "ok":
